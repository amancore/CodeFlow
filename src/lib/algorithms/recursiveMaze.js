...
function recursiveMazeVertical(
	pairs,
	board,
	startRow,
	endRow,
	...
	const mid = Math.floor((endCol + startCol) / 2);
	let random = Math.floor(Math.random() * (endRow - startRow + 1)) + startRow;

	let start = startRow;
	if (
		!board[startRow - 1] ||
		!board[startRow - 1][mid] ||
		!board[startRow - 1][mid].isWall
	) {
		random = start;
		start++;
	}
...
function recursiveMazeHorizontal(
	...
	const mid = Math.floor((endRow + startRow) / 2);
	let random = Math.floor(Math.random() * (endCol - startCol + 1)) + startCol;

	let start = startCol;
	if (
		!board[mid] ||
		!board[mid][startCol - 1] ||
		!board[mid][startCol - 1].isWall
	) {
		random = start;
		start++;
	}
